...
StoreComment<-data.frame(matrix(NA,NumberofSamples,2))
for(i in 1:NumberofSamples){
  ...
  StoreComment[i,1]<-str_split_i(str_replace_all(toString(QuantResult[which(QuantResult=='Comment',arr.ind=T)[1],]),"([,])",""),"Comment",1)
  StoreComment[i,2]<-str_replace_all(str_split_i(str_replace_all(toString(QuantResult[which(QuantResult=='Comment',arr.ind=T)[1],]),"([,])",""),"Comment",2),"([:])","")
}
colnames(StoreKraws)<-elementlabels[2:length(elementlabels)]
colnames(StorePositions)<-c("X","Y","Z","Rel Dist")
colnames(StoreComment)<-c("Position No.","Comment")
#write.csv(StoreKraws,file=file.path(getwd(),paste(FolderName,"RStudio_Extract_Kraws_Points.csv",sep="_")))
write.csv(cbind(StoreKraws,StorePositions,StoreComment),file=file.path(getwd(),paste(FolderName,"RStudio_Extract_Kraws_Points_RelDist_Comment.csv",sep="_")))
